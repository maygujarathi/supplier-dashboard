...
<style>
    ...
    * {
    ...
    div[data-testid="stVerticalBlockBorderWrapper"] {
        background: linear-gradient(135deg, rgba(30, 41, 59, 0.55) 0%, rgba(25, 35, 50, 0.35) 100%);
...
def calculate_score(row):
    s = 100.0
    if pd.notna(row.get("Liefertreue")) and row["Liefertreue"] < 95:
        s -= (95 - row["Liefertreue"]) * 1.8
    if pd.notna(row.get("Lieferzeit")) and row["Lieferzeit"] > 10:
        s -= (row["Lieferzeit"] - 10) * 2.0
    if pd.notna(row.get("Qualitätsrate")) and row["Qualitätsrate"] < 97:
        s -= (97 - row["Qualitätsrate"]) * 2.5
    if pd.notna(row.get("Reklamationsquote")) and row["Reklamationsquote"] > 1.0:
        s -= (row["Reklamationsquote"] - 1.0) * 8
    if pd.notna(row.get("Preisabweichung")) and abs(row["Preisabweichung"]) > 1.0:
        s -= (abs(row["Preisabweichung"]) - 1.0) * 3.5
...
def supplier_strengths(row):
    ...
    if row["Liefertreue"] >= 95:
        r.append("strong delivery")
    if row["Qualitätsrate"] >= 97:
        r.append("excellent quality")
    if row["Lieferzeit"] <= 10:
        r.append("fast lead time")
    if row["Reklamationsquote"] <= 1.0:
        r.append("low complaints")
    if abs(row["Preisabweichung"]) <= 1.0:
        r.append("stable pricing")
...
def donut_chart(value, color):
    ...
    fig.update_layout(
        ...
        font=dict(family="Inter, Arial, sans-serif", color="#d1d5db"),
        annotations=[dict(
...
def plotly_dark_layout(**kwargs):
    base = dict(
        ...
        margin=dict(t=62, b=62, l=52, r=42),
        xaxis=dict(
            title_font=dict(size=12, color="#d1d5db"),
        ...
        coloraxis_colorbar=dict(
            ...
            title=dict(font=dict(size=12, color="#f3f4f6"))
        )
    )
    base.update(kwargs)
    return base
...
df["Anomalien"] = (
    df["Anomaly_Flag"].apply(normalize_anomaly_flag)
    if "Anomaly_Flag" in df.columns
    else df.apply(calculate_anomalies, axis=1)
)
...
with tab3:
    ...
    with r2c2:
        with st.container(border=True):
            ...
            fig6.update_layout(**plotly_dark_layout())
...
**Critical KPI:** {sel['Crit_KPI']}  
        ...
        if sel["Score"] >= 90:
            st.markdown(
                f'<div class="insight-box"><b>✅ Recommendation</b><br>{sel["Supplier"]} is a preferred supplier. Strengths: {sel["Strengths"]}</div>',
                unsafe_allow_html=True
            )
        elif sel["Score"] >= 75:
            st.markdown(
                f'<div class="warning-box"><b>⚠️ Recommendation</b><br>Monitor <b>{sel["Crit_KPI"]}</b> closely. Supplier is usable but requires oversight.</div>',
                unsafe_allow_html=True
            )
        else:
            st.markdown(
                f'<div class="critical-box"><b>🔴 Critical</b><br>Focus on <b>{sel["Crit_KPI"]}</b>. Consider alternative suppliers.</div>',
                unsafe_allow_html=True
            )
...
with tab5:
    ...
    st.dataframe(
        agg[[c for c in detail_cols if c in agg.columns]].round(2),
        use_container_width=True,
        hide_index=True
    )
