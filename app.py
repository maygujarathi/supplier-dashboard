...
def plotly_theme(**overrides):
    """Base Plotly layout.

    Important: use this helper with overrides instead of calling
    fig.update_layout(**plotly_theme(), margin=...) because duplicate keyword
    arguments can crash Streamlit Cloud.
    """
    base = dict(
...
with fc3:
    statuses = ["All Statuses"] + sorted(filt_status for filt_status in agg["Status"].dropna().astype(str).unique().tolist())
...
if sel_status != "All Statuses":   filt = filt[filt["Status"].astype(str) == sel_status]
...
with left_col:
    ...
    with ch2:
        ...
        fig_ctry.update_layout(**plotly_theme(
            height=200,
            showlegend=False,
