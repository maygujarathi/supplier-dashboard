...
# =========================================================
# HEADER  — fixed: clamp font, no overflow
# =========================================================
...
# =========================================================
# VALIDATE & BUILD DATAFRAME  (runs on every rerun — no button gate)
# =========================================================
required_map = {
...
if missing:
    st.error(f"⚠️ Please map all required fields in the sidebar: {', '.join(missing)}")
    st.stop()
...
# =========================================================
# SIDEBAR — FILTERS  (live, no button needed)
# =========================================================
with st.sidebar:
    ...
    # ── FIX: keep material selection in session_state so it survives reruns ──
    ...
    # Apply material filter immediately
    filtered = agg if sel_mat == "All" else agg[agg["Material"] == sel_mat].copy()

    # Build supplier list from the already-filtered dataframe
    supplier_list = filtered["Supplier"].tolist()

    # ── FIX: reset supplier selection when it no longer exists in filtered list ──
    if "sel_sup" not in st.session_state or st.session_state["sel_sup"] not in supplier_list:
        st.session_state["sel_sup"] = supplier_list[0] if supplier_list else None
